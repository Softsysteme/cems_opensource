...
//		androidSenderID: ""
	 ...
	 */
	vars: {
		fs_default_db: "retaildb",
		fs_default_design: "design",
...
//		},
		{
			calledRequest: ".select_shop",
			actions: [
			    {
			    	condition: ">shopCode",
			    	beforeRendering: function ($doc, c8oData) {
			    		App.shopCode = $doc.find(">shopCode").text();
			    		...
			    		// Start the replication pull
			    		C8O.call("fs://.replicate_pull", options);
			    	}
			    },
			    {
			    	condition: ":not(>shopCode)",
			    	goToPage : "init.html"
			    }
			]
		},
		{
			calledRequest: "fs://.replicate_pull",
			actions : [ 
				{
	           		condition: ">event",
	           		beforeRendering : function ($doc, c8oData) {
	           			...
	           			//Redirect to the optimization page when complete
		...
		{
			calledRequest: ".fill_server_database",
            actions: [
				{
					afterRendering: function ($doc, c8oData) {
						C8O.call(".select_shop", {shopCode : App.shopCode} );
					},
					goToPage : "app.html"
				}
			]
		...
		{
            calledRequest: "fs://.get",
            actions: [
				{
					condition : function ($doc, c8oData) {
						return true;
					},
					goToPage : "product_detail.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = -3;
					}
				}
			]
        },
		{
            calledRequest: "fs://.view",
            actions: [
				{
					// Level * products
					condition : function ($doc, c8oData) {
						...
						var isProduct = false;
						try {
							isProduct = $doc.find("rows").children().eq(0).find("sku").length > 0;
						}
						catch(err) {
						    return false;
						}
						
						return c8oData.view == "children_byFather" && isProduct;
					},
					goToPage : "products_list.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = -2;
					}
				},
				{
				  	// Level 1 categories (root)
					condition : function ($doc, c8oData) {
						var gotResult = $doc.children() && $doc.find("rows") && $doc.find("rows").children() && $doc.find("rows").children().length > 0;
						return c8oData.root == "" && c8oData.view == "children_byFather" && gotResult;
					},
					goToPage : "categories_tile_level1.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = 1;
    				}
				},
				{
				  	// Level 2 categories
					condition : function ($doc, c8oData) {
    					return c8oData.view == "children_byFather";
    				},
					fromPage : "#categories_tile_level1",
					goToPage : "categories_tile_level2.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = 2;
    				}
				},
				{
					...
					fromPage : "#categories_tile_level2",
					goToPage : "categories_list_level3.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = 3;
    				}
				},
				{
					...
					fromPage : "#categories_list_level3",
					goToPage : "categories_list_level4.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = 4;
    				}
				},
				{
				  	// Level 5 categories
					...
					fromPage : "#categories_list_level4",
					goToPage : "categories_list_level5.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = 5;
    				}
				},
				{
				  	// Level 6 categories
					...
					fromPage : "#categories_list_level5",
					goToPage : "categories_list_level6.html",
					beforeRendering : function ($doc, c8oData) {
    					ProjectData.requiredDepthLevel = 6;
    				}
				}
            ]
        },
...
var ProjectData = {
	...
	/**
	 * Translate an index to the corresponding block type. (index%4==0 -> a, index%4==1 -> b, ...)
	 */
	indexToBlockType : function (index) {
		return String.fromCharCode(97 + (index % 4));
	},
	checkImageUrl : function (imageUrl) {
		// C8O.log.warn("imageUrl " + imageUrl);
		if (imageUrl == "") {
			return "default.jpg";
		}
	...
	checkDepthLevel1 : function ($doc, c8oData) {
		if (c8oData.depthLevel) {
			return (c8oData.depthLevel == 1);
		}
		return false;
	},
	checkDepthLevel2 : function ($doc, c8oData) {
		if (c8oData.depthLevel) {
			return (c8oData.depthLevel == 2);
		}
		return false;
	},
	checkDepthLevel3 : function ($doc, c8oData) {
		if (c8oData.depthLevel) {
			return (c8oData.depthLevel == 3);
		}
		return false;
	},
	checkDepthLevel4 : function ($doc, c8oData) {
		if (c8oData.depthLevel) {
			return (c8oData.depthLevel == 4);
		}
		return false;
	},
	checkDepthLevel5 : function ($doc, c8oData) {
		if (c8oData.depthLevel) {
			return (c8oData.depthLevel == 5);
		}
		return false;
	},
	checkDepthLevel6 : function ($doc, c8oData) {
		if (c8oData.depthLevel) {
			return (c8oData.depthLevel == 6);
		}
		return false;
	},
	checkDepthLevelProductsList : function ($doc, c8oData) {
		return (ProjectData.requiredDepthLevel == -2);
	},
	checkDepthLevelProductDetail : function ($doc, c8oData) {
		return (ProjectData.requiredDepthLevel == -3);
	}
}
