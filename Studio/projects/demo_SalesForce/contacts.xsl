...
<xsl:template match="document">
	<body onload="twsInit()" defer="true" >
		...
		<!-- 
		| This lines will organize some header text at the top of the cliplet
		-->
		<table border="0" width="100%" cellpadding="0" cellspacing="0">
			<tr>
				<td align="left">
					<xsl:apply-templates select="DIV[@class='rolodex']"/>
				</td>
				<td align="right">
					<xsl:apply-templates select="DIV[@class='next']"/>
				</td>
			</tr>
		</table>
		...
		-->
		<xsl:apply-templates select="HEAD/following-sibling::*[@class!='rolodex' and @class!='next' and @class!='sidebarModuleHeader' and @class!='standardSearchElementBody']" />
...
<xsl:template match="TH[position() = 4]" />								<!-- remove the fourth column title  -->
<xsl:template match="TD[position() = 3]" />		  						<!-- remove the fourth column  	     -->

<xsl:template match="TH[position() = 7]" />								<!-- remove the seventh column title -->
<xsl:template match="TD[position() = 6]" />		  						<!-- remove the seventh column  	 -->
