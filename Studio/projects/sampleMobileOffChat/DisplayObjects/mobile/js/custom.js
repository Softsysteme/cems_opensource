...
//        first_call: "false", /** true/false: automatically call convertigo using the page query/hash parameters, after the init_finished hook */
        ...
        fs_default_db: "offchat_fullsync",
		fs_default_design: "OffChat"
            ...
            actions: [
                {
    				...
    				beforeRendering: function ($doc, c8oData) {
    					$(".user").text(c8oData.__user).val(c8oData.__user);
    				}
                }
            ]
        }
...
C8O.addHook("call", function (data) {
	if (data.__sequence == "post" && data.num) {
		data.num *= 1;
		var img = $("#messageForm img").attr("src");
		if (img) {
			data._attachments = {
				"img.jpeg": {
			...
			$("#messageForm img").removeAttr("src").hide();
		}
	}
    return true;
});
...
C8O.addHook("init_finished", function (params) {
	...
	$(document).on("change", "#username, #password", function () {
	...
	}).on("click", "#makePhoto", function () {
		if (navigator.camera) {
		    navigator.camera.getPicture(function (imageData) {
				$("#messageForm img").attr("src", "data:image/jpeg;base64," + imageData).show();		    	
		    }, function (err) {
		    	console.log(err);
		    }, {
		    	quality: 80,
		...
		if (video) {
			...
			video = $("#chat video").on("canplay", function (ev) {
				height = this.videoHeight / (this.videoWidth / width);
				video.setAttribute("width", canvas.width = width);
				video.setAttribute("height", canvas.height = height);
			}).on("playing", function (ev) {
				window.setTimeout(function () {
					canvas.getContext("2d").drawImage(video, 0, 0, width, height);
					var data = canvas.toDataURL("image/jpeg");
					$("#messageForm img").attr("src", data).show();
					video.pause();
				}, 200);
			})[0];
