...
// Implements the XML  Ajax POST on a context;
function ajaxXmlPostDataContext(xmlRequester, data, contextName) {
	contextN = contextName;
	ajaxXmlPostData(xmlRequester, data);
}
...
function ajaxXmlPostData(xmlRequester, data) {
   if (xmlRequester) {
   	...
   	if (contextN != null && contextN != "") 
	   	xmlRequester.send(data + "&__context=" + contextN);
	else
