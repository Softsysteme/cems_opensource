...
Ext.apply(app, {
	...
	checkConfiguration: function () {
		...
		if (!app.logged && Ext.isDefined(app.panels.config)) {
			// display a popout error message
			Ext.Msg.alert("Error","Configuration validation required", function() {
				// switch to login panel
				app.activatePanel('config');
			});
			return false;
		}
		return true;
	...
	/**
	 * Usefull method to open a new browser window with the given url parameter
	 */
	openLinkInNewWindow: function(url){
		var link = Ext.getDom('hidden_link'); // see index.html
		link.href = url;
...
Ext.setup({
	...
	onReady: function () {
		...
		Ext.apply(app, {
			...
			// like the defaultMask but automatically displayed when the 'homeStore' is loading data (see feedsPanel.js)
			feedsMask: new Ext.LoadMask(Ext.getBody(), {
				msg: 'Loading feed list, please wait ...',
				store: 'homeStore'
			}),
			// like the defaultMask but automatically displayed when the 'feedStore' is loading data (see feedPanel.js)
			feedMask: new Ext.LoadMask(Ext.getBody(), {
