...
//		loading: {} /** loading option object argument for the $.mobile.loading("show") called by C8O.waitShow() */
	...
	routingTable: [
		...
		{
			calledRequest: ".loadFeedsTree",
			actions: [
				{
					condition: ">items",
					goToPage: "#home",
					afterRendering: function($doc, c8oData){
						$doc.find(">items").each(function(){
							var countItems = $(this).find(">items").length;
						});
					}
				}
			]
...
/*******************************************************
 * Functions templating / afterRendering *
 *******************************************************/

/*******************************************************
 * Functions templating / beforeRendering (edit response xml dom) *
 *******************************************************/

/*******************************************************
 * Functions templating / Formatter, listen-condition in html *
 *******************************************************/
//count number of child items from principal items
...
function feedIMG(value) {
	return value != "javascript:void(0);" ? "show" : "hide";
}
/*******************************************************
 * Functions *
 *******************************************************/
...
C8O.addHook("document_ready", function () {
	$(".subhome,.submenu,.subitem").hide();
	$(document).on("click", ".homeItem" ,function(){
		var title = $(this).attr("title");
		$("div[title=" + title + "]").show();
		$("#menu").hide();
	});
	$(document).on("click", ".subhomeItem", function(){
		var subhometitle = $(this).attr("subhometitle");
		$("div[subhometitle=" + subhometitle + "]").show();
		$("#menu, #subhome").hide();
	});
	$(document).on("click", ".submenuItem", function(){
		var submenutitle = $(this).attr("submenutitle");
		$("div[submenutitle=" + submenutitle + "]").show();
		$("#menu, #subhome, #submenu").hide();
	});
	$(document).on("click", ".submenuItem", function(){
		/*var submenutitle = $(this).attr("submenutitle");
		$("div[submenutitle=" + submenutitle + "]").show();
		$("#menu, #subhome, #submenu, #subitem").hide();*/
	});
	return true;
});
