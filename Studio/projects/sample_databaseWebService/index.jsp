...
<%		String uri = request.getRequestURI();
		...
		int firstSlach 		= 0;
		int secondSlach 	= uri.indexOf("/", firstSlach + 1);
		int thirdSlach 		= uri.indexOf("/", secondSlach + 1);
		int begin 			= thirdSlach + 1;
		...
		Project project;
		Vector connectors, transactions, variablesDef, variable, sequences, seqVariablesDef, seqVariable;
		Connector connector;
...
%>
												...
												<tr>
													...
													<td valign="top" class="table-row<%=i%>">
														<a href="#" onclick="launchCliplet('form_<%=k%>');">
													...
													<td valign="top" class="table-row<%=i%>">
<%						variablesDef = transaction.getVariablesDefinition();
						if (variablesDef.size() > 0) {
%>
														<table width="100%">
<%							for (int m = 0 ; m < variablesDef.size() ; m++) {
								variable = (Vector) variablesDef.elementAt(m);
								variableName = (String) variable.elementAt(0);
								variableDesc = (String) variable.elementAt(1);
								variableDefaultValue = (String) variable.elementAt(2);
%>
															<tr>
																<td width="30%" class="table-row<%=i%>"><span class="small"><%=variableName %></span></td>
																<td width="30%" class="table-row<%=i%>"><span class="small"><input type="text" class="small" name="form_<%=k%>_input" variable="<%=variableName %>" value="<%=variableDefaultValue %>" /></span></td>
...
%>
												...
												<tr>
													...
													<td valign="top" class="table-row<%=i%>">
<%					seqVariablesDef = sequence.getVariablesDefinition();
					if (seqVariablesDef.size() > 0) {
%>
														<table width="100%">
<%						for (int n = 0 ; n < seqVariablesDef.size() ; n++) {
							seqVariable = (Vector) seqVariablesDef.elementAt(n);
							seqVariableName = (String) seqVariable.elementAt(0);
							seqVariableDesc = (String) seqVariable.elementAt(1);
							seqVariableDefaultValue = (String) seqVariable.elementAt(2);
%>
															<tr>
