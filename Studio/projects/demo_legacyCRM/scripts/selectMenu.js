...
function show_mySelectMenu(e, curField, menu) {
	var tabHeight = 1*menu.style.height.replace("px", "");
	var dispo = document.documentElement.clientHeight - 5;
	
	if (tabHeight > dispo) {
		menu.style.height = dispo + "px";
	}
	else {
		alert(menu.getAttribute('origheight'));
		menu.style.height = menu.getAttribute("origheight");
	}
	if (isIE) {
		...
		menu.style.top = Math.max(document.documentElement.scrollTop, (e.clientY + document.documentElement.scrollTop - tabHeight)) + "px";
	}
	else {
		//alert(dispo);
		menu.style.left = e.pageX + "px";
		//menu.style.top = e.pageY + "px";
		menu.style.top = Math.max(document.documentElement.scrollTop, (e.pageY - tabHeight)) + "px";
	}
...
function updateDataInMultiSelected(data, menu) {
	if (multiSelect.length > 0) {
		for (i=0; i<multiSelect.length; i++) {
			getId(multiSelect[i]).value = data;
		}
		doAction("KEY_ENTER");
	}
	else {
		updateDataInObjSelected(data, menu);
	}
	menu.style.visibility = "hidden";
}