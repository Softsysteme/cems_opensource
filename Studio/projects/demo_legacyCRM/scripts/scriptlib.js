...
var isGC = navigator.userAgent.indexOf('Chrome') == -1 ? true: false;
...
function setXSLparam(name,value){
	var elements = xslDom.getElementsByTagName(((isGC)?"xsl:":"")+"param");
	//var elements = xslDom.getElementsByTagName("xsl:param");
...
function focusPrevField(){
	if (document.javelin_form.elements.length > 0) {
		...
		while (myid > 0) {
			myid--;
			//alert("analyzing myid='" + myid + "'");
			//if (isField(document.javelin_form.elements[myid]) && document.javelin_form.elements[myid].type != 'hidden') {
			if (isFocusable(document.javelin_form.elements[myid])) {
				//alert("is a focusable field : '" + document.javelin_form.elements[myid].id + "'");
				//focusOnField=document.javelin_form.elements[myid];
...
function focusNextField(){
	if (document.javelin_form.elements.length > 0) {
		//alert("currentFieldOnFocus='" + currentFieldOnFocus + "'");
		var myid = javelin_formElements[currentFieldOnFocus];
		while (myid < document.javelin_form.elements.length-1) {
			...
			//if (isField(document.javelin_form.elements[myid]) && document.javelin_form.elements[myid].type != 'hidden') {
			if (isFocusable(document.javelin_form.elements[myid])) {
				//alert("is a field : '" + document.javelin_form.elements[myid].id + "'");
				//focusOnField=document.javelin_form.elements[myid];
				document.javelin_form.elements[myid].focus();
				if (isSelectable(document.javelin_form.elements[myid])) {
					document.javelin_form.elements[myid].select();
