...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="tablex[@type='table']">
		...
		<!-- Main container -->
		<div class="Grid"
			 style="border-left: solid 1px #0099cc;
		...
		<span style="position:absolute;
			...
			<br/><br/><img src="images/spec/excel.png" alt=""  onclick="window.open('datascsv.xls', '_blank');" onmouseover="tooltip.show(this)" onmouseout="tooltip.hide(this)" title="Excel export" style="cursor: pointer;"/>
							...
							<span id="{@name}_n{../@page}parent">
								<xsl:choose>
									<xsl:when test="name() = 'X'">
										<img src="images/spec/selectarrow.png"
												style="cursor: pointer;"
												onclick="getId('__javelin_current_field').value = '{@name}'; currentFieldOnFocus = '{@name}'; doAction('KEY_ENTER');"/>
									</xsl:when>
									<xsl:otherwise>
										<input type="text"
												class="fieldText"
	...
	<xsl:template match="Title" priority="1">
		<td  style="width:{round(@size * $coefx)}px;height:{../../@titleheight * $coefy}px;" class="datatitle" valign="center" onclick="onSortClicTD({position()})">
			<xsl:if test="string-length(.) = 0">&#160;</xsl:if>
			<xsl:if test="string-length(.) != 0">
	...
	<xsl:template match="actionsTable" priority="1">
		<div class="actDiv" id="act_c{../@column}_l{../@line}" style="height: {count(action)*$coefy}px; z-index:12; overflow-y:scroll; overflow-x:hidden;" origheight="{count(action)*$coefy}px">
