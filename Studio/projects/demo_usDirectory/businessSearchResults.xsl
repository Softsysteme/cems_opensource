...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lxslt="http://xml.apache.org/xslt">
    ...
    <xsl:template name="styles">
        <style>
            <![CDATA[
                body { background-color: #FFF; margin:0px; padding: 0px; color:#818181; font-size: 11px;}
	...
	<xsl:template match="document">
		<body onload="twsInit()" defer="true" style="background: #FFF none repeat scroll 0 0" >
			<!-- <xsl:apply-templates select="HEAD/LINK"/>-->
            <!--<link type="text/css" href="css/default.css" rel="styleSheet" />-->
            <xsl:call-template name="styles"/>
			<xsl:apply-templates select="HEAD/following-sibling::*" />
			
			<!-- applying templates on errorMessage if existing -->
	...
	<xsl:template match="SPAN[@class='result_item']">
    
        <xsl:variable name="res_pos" select="count(preceding-sibling::SPAN[@class='result_item'])+1"/>
        <xsl:variable name="res_name" select="(//results/item/name)[$res_pos]"/>
        <xsl:variable name="tmp_addr" select="(//results/item/address)[$res_pos]"/> 
        ...
        <xsl:variable name="res_zip" select="normalize-space(substring-after($tmp_addr3, ' '))"/>
            ...
            <tr>
                <td style="border-top: 1px solid #00a4c8; border-left: 1px solid #00a4c8 ; padding: 10px 0 0 10px;">
                ...
                <td rowspan="2" style="border-top: 1px solid #00a4c8 ;border-bottom: 1px solid #00a4c8 ;border-right: 1px solid #00a4c8 ; padding: 10px 10px 0 10px;">
                    <xsl:copy>
            ...
            <tr>
                <td style="border-bottom: 1px solid #00a4c8; border-left: 1px solid #00a4c8 ; padding: 5px 0 10px 10px;">
	...
	<xsl:template match="SPAN[@class='phone_button']" />
	
	<xsl:template match="cliperror" priority="2">
		<SPAN><xsl:value-of select="."/></SPAN>
	</xsl:template>
		
	<xsl:template match="rowindex"/>	<!-- don't display the row index added by convertigo -->
	<xsl:template match="results"/>		<!-- don't display the results added by convertigo -->
</xsl:stylesheet>
